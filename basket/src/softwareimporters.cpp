...
Note* SoftwareImporters::insertTitledNote(BasketScene *parent, const QString &title, const QString &content, Qt::TextFormat format/* = Qt::PlainText*/, Note *parentNote/* = 0*/)
{
    ...
    nTitle->addState(Global::tagModel->stateForId("title"));
...
{
    QString dirPath = QStandardPaths::standardLocations(QStandardPaths::DataLocation).at(0);
...
{
    QUrl url = QFileDialog::getOpenFileUrl(0, QCoreApplication::tr("KnowIt files"), QUrl(), "KnowIt files (*.kno);; All files (*)");
    if (!url.isEmpty()) {
        QFile file(url.path());
...
void SoftwareImporters::importTuxCards()
{
    ...
    if (document == 0) {
        QMessageBox::critical(0, QCoreApplication::tr("Bad File Format"), QCoreApplication::tr("Can not import that file. It is either corrupted or not a TuxCards file."));
        return;
    }
